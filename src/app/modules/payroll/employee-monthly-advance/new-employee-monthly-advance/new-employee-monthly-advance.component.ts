...
export class NewEmployeeMonthlyAdvanceComponent implements OnInit {
  ...
  get filteredFormArray(): any[] {
    ...
    if (!formArray || !this.employeeNameFilter) return formArray?.controls || [];
    const filter = this.employeeNameFilter.toLowerCase();
    return formArray.controls.filter((c: any) => {
      const name = c.get('EMP_NAME')?.value?.toLowerCase() || '';
      const code = c.get('EMP_CODE')?.value?.toLowerCase() || '';
      return name.includes(filter) || code.includes(filter);
    });
  }

  isRowVisible(i: number): boolean {
    if (!this.employeeNameFilter) return true;
    const formArray = this.dynamicForm?.get('formArray') as FormArray;
    const control = formArray?.controls[i];
    if (!control) return true;
    const filter = this.employeeNameFilter.toLowerCase();
    const name = control.get('EMP_NAME')?.value?.toLowerCase() || '';
    const code = control.get('EMP_CODE')?.value?.toLowerCase() || '';
    return name.includes(filter) || code.includes(filter);
  }
